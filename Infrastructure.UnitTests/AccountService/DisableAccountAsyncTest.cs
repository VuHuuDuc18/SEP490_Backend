...
{
    public class DisableAccountAsyncTest
    {
        ...
        public DisableAccountAsyncTest()
        {
        ...
        [Fact]
        public async Task DisableAccountAsync_Successful()
        {
            // Arrange
            ...
            // Act
            var result = await _service.DisableAccountAsync(email);
        ...
        [Fact]
        public async Task DisableAccountAsync_UserNotFound()
        {
            // Arrange
        ...
        [Fact]
        public async Task DisableAccountAsync_UserAlreadyDisabled()
        {
            // Arrange
            var email = "test@example.com";
            var user = new User
            ...
            // Assert
            Assert.False(result.Succeeded);
            Assert.Equal($"Tài khoản đã bị vô hiệu hóa - {email}.", result.Message);
            Assert.Null(result.Data);
            Assert.False(user.IsActive);
            _userManagerMock.Verify(x => x.FindByEmailAsync(email), Times.Once());
            _userManagerMock.Verify(x => x.UpdateAsync(It.IsAny<User>()), Times.Never());
