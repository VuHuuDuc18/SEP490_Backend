...
namespace Infrastructure.UnitTests.AccountService
{
    public class DeleteAccountTest
    {
        ...
        [Fact]
        public async Task DeleteAccount_Successful()
        {
            // Arrange
            ...
            var options = new DbContextOptionsBuilder<TestIdentityDbContext4>()
                .UseInMemoryDatabase(databaseName: Guid.NewGuid().ToString())
                .Options;
            using var context = new TestIdentityDbContext4(options);
            context.Users.Add(user);
            await context.SaveChangesAsync();

            _userManagerMock.Setup(x => x.FindByEmailAsync(email))
                .ReturnsAsync(context.Users.FirstOrDefault(u => u.Email == email));
            _userManagerMock.Setup(x => x.DeleteAsync(user))
                .ReturnsAsync(IdentityResult.Success)
                .Callback<User>(u => context.Users.Remove(u));
    ...
    {
        public TestIdentityDbContext4(DbContextOptions<TestIdentityDbContext4> options) : base(options) { }
